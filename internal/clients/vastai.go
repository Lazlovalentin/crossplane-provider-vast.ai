...
const (
	...
	errUnmarshalCredentials = "cannot unmarshal vastai credentials as JSON"
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		// Wire Vast.ai credentials into the Terraform provider configuration.
		// The vastai TF provider accepts api_key (required) and api_url (optional,
		// defaulting to https://console.vast.ai).
		cfg := map[string]any{}
		if v, ok := creds[keyAPIKey]; ok && v != "" {
			cfg[keyAPIKey] = v
		}
		if v, ok := creds[keyAPIURL]; ok && v != "" {
			cfg[keyAPIURL] = v
		}
		if _, ok := cfg[keyAPIKey]; !ok {
			return ps, errors.New("vastai credentials missing required field: api_key")
		}
